            ...
            if (
                ...
                if is_limit and non_immediate_stop:
                    if non_immediate_stop_direction_up:
                        ...
                        limit_before_stop = (not is_long_order) and (
                            pending_order_price <= non_immediate_stop_price
                        )
                    else:
                        # Price going down to stop; long limit (higher price) fills first
                        limit_before_stop = is_long_order and (
                            pending_order_price >= non_immediate_stop_price
                        )
                ...
                if immediate_stop:
                    ...
                    stop_price_out[i - 1] = (
                        immediate_stop_price  # i - 1 means it's in the order row, which is consistent with vectorized backtesting
                    )
                    ...
                    is_position_close = True
                    stop_price_out[i - 1] = (
                        non_immediate_stop_price  # i - 1 means it's in the order row, which is consistent with vectorized backtesting
                    )
                elif is_limit:
                ...
                if has_fill:
                    ...
                    if is_position_close:
                        ...
                        if position == 0.0:
                            ...
                            is_adding = (trade_size > 0.0 and position > 0.0) or (
                                trade_size < 0.0 and position < 0.0
                            )
                            if is_adding:
            ...
            if can_order:
                ...
                if long_only and sig == -1.0:
                    ...
                    if not np.isnan(order_px) and not np.isnan(order_qty):
                        ...
                        if position != 0.0:
                            if (position > 0.0 and sig < 0.0) or (
                                position < 0.0 and sig > 0.0
                            ):
                                order_size += -position
                        has_order = True
