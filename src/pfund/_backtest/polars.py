...
class BacktestDataFrame(NarwhalsMixin, pl.DataFrame):
    ...
    def create_signal(
        ...
        if signal is None:
            ...
            assert signal.drop_nulls().unique().is_in([1, -1]).all(), (
    ...
    def open_position(
        ...
        if order_price is None:
            ...
            assert ((order_price > 0) | order_price.is_null()).all(), (
                "'order_price' must be positive or null"
            )
        if isinstance(order_quantity, pl.Series):
            assert ((order_quantity > 0) | order_quantity.is_null()).all(), (
                "'order_quantity' values must be positive or null"
            )
        else:
        ...
        regular_market_condition = (long_order & (prev_close <= opened_order_price)) | (
        ...
        gap_through_market_condition = (
            ...
            & (pl.col("open") < opened_order_price)
        ) | (
            short_order
            & ~regular_market_condition
            & (pl.col("open") > opened_order_price)
        ...
        market_order_trade_condition = (
            regular_market_condition | gap_through_market_condition
        )
        ...
        df = df.with_columns(
            trade_price=(
                pl.when(gap_through_market_condition)
                .then(pl.col("open"))
                .when(regular_market_condition)
                .then(regular_market_fill)
                .when(limit_order_trade_condition)
                .then(opened_order_price)
                .otherwise(None)
            ),
        ...
        if first_only or long_only or short_only:
            ...
            filtered_trades = filtered_orders.shift(1).fill_null(
                False if opposite_side is not None else True
            )
            ...
            df = df.with_columns(
                order_size=pl.when(order_mask)
                .then(None)
                .otherwise(pl.col("order_size")),
                order_price=pl.when(order_mask)
                .then(None)
                .otherwise(pl.col("order_price")),
                trade_size=pl.when(trade_mask)
                .then(pl.col("trade_size"))
                .otherwise(None),
                trade_price=pl.when(trade_mask)
    ...
    def close_position(
        ...
        def _calculate_stop_price(_df: pl.DataFrame) -> pl.DataFrame:
            ...
            market_order_trade_condition = (
                (
                    long_order
                    & (
                        (prev_close <= opened_order_price)
                        | (opened_order_price > pl.col("high"))
                    )
                )
                | (
                    short_order
            ...
            for tp_or_sl, sign in [(stop_loss, -1), (take_profit, 1)]:
                if tp_or_sl is None:
                    continue
                stop_price_computed = pl.col("avg_price") * (
                    1 + end_position_side * tp_or_sl * sign
                )
                ...
                gap_through_stop = (
                    negative_sign
                    & ~regular_immediate_stop
                    & (pl.col("open") < opened_stop_order_price)
                ) | (
                    positive_sign
                    & ~regular_immediate_stop
                    & (pl.col("open") > opened_stop_order_price)
                ...
                stop_market_order_triggered_condition_immediate = (
                    regular_immediate_stop | gap_through_stop
                )
                ...
                is_limit_order_filled_first = limit_order_trade_condition & (
                    (
                        negative_sign
                        & long_order
                        & (opened_order_price >= opened_stop_order_price)
                    )
                    | (
                        positive_sign
                        & short_order
                ...
                should_set_stop = (
                    stop_market_order_trade_condition & pl.col("stop_price").is_null()
                )
                _df = _df.with_columns(
                    pl.when(should_set_stop)
                    .then(opened_stop_order_price)
                    .otherwise(pl.col("stop_price"))
                    .alias("stop_price"),
                    pl.when(should_set_stop)
                    .then(stop_market_order_triggered_condition_immediate)
                    .otherwise(pl.col("_immediate_stop"))
                    .alias("_immediate_stop"),
                    pl.when(should_set_stop)
                    .then(gap_through_stop)
                    .otherwise(pl.col("_gap_through_stop"))
                    .alias("_gap_through_stop"),
                )

            _df = _df.with_columns(
                stop_price=pl.col("stop_price").shift(
                    -1
                ),  # shift back stop_price from trade row to order row
                _immediate_stop=pl.col("_immediate_stop").shift(-1).fill_null(False),
                _gap_through_stop=pl.col("_gap_through_stop")
                .shift(-1)
                .fill_null(False),
            )
        ...
        df = df.with_columns(position=(global_cumsum - streak_start_cumsum))
        ...
        df = df.with_columns(_agg_costs=(global_cost_cumsum - streak_start_cost_cumsum))
        df = df.with_columns(
            avg_price=(
        ...
        df = df.with_columns(avg_price=pl.col("avg_price").forward_fill())
        ...
        if has_close_condition:
            ...
            if take_profit or stop_loss:
                not_time_window = (
                    ~pl.col("_time_window") if time_window else pl.lit(True)
                )
                price_diff_check = pl.col("position").sign() * (
                    pl.col("avg_price") - pl.col("stop_price")
                )
                df = df.with_columns(
                    _stop_loss=(
                        pl.col("_first_stop_order")
                        & not_time_window
                        & (price_diff_check > 0)
                    ),
                    _take_profit=(
            ...
            stop_fill = (
                pl.when(is_gap_stop_trade).then(pl.col("open")).otherwise(stop_fill)
            )
            # Regular immediate stops and time_window close orders follow fill_price policy
            is_market_stop_trade = (
                pl.col("_immediate_stop").shift(1).fill_null(False) & ~is_gap_stop_trade
            )
            if time_window:
                is_market_stop_trade = is_market_stop_trade | pl.col(
                    "_time_window"
                ).shift(1).fill_null(False)
            if fill_price == "open":
                stop_fill = (
                    pl.when(is_market_stop_trade)
                    .then(pl.col("open"))
                    .otherwise(stop_fill)
                )
            ...
            df = df.with_columns(
                trade_size=pl.when(first_stop_trade)
                .then(offset_size)
                .otherwise(pl.col("trade_size")),
                trade_price=pl.when(first_stop_trade)
                .then(stop_fill)
            ...
            order_mask = (
                first_stop_order_forwards_mask
                & pl.col("signal").eq(stop_side_ffill.sign())
                & pl.col("stop_price").is_null()
            )
            position_mask = first_stop_order_forwards_mask & ~pl.col(
                "_first_stop_order"
            )
        ...
        df = df.with_columns(
            avg_price=pl.when(pl.col("position").eq(0))
            .then(None)
        ...
        df = df.with_columns(
            trade_size=(
                pl.when(
                    pl.col("_position_change")
                    & offset_trade_size.ne(0)
                    & pl.col("stop_price").shift(1).is_null()
                )
            ...
            order_size=(
                pl.when(
                    offset_order_size.sign().eq(pl.col("signal"))
                    & offset_order_size.ne(0)
                )
