...
class BacktestDataFrame(NarwhalsMixin, pd.DataFrame):
    ...
    def create_signal(
        ...
        if signal is None:
            ...
            assert pd.Series(signal.unique()).dropna().isin([1, -1]).all(), (
                "'signal' must only contain 1, -1, nan"
            )
            self["signal"] = signal
    ...
    def open_position(
        ...
        if order_price is None:
            ...
            assert ((order_price > 0) | order_price.isna()).all(), (
                "'order_price' must be positive or nan"
            )
        if isinstance(order_quantity, pd.Series):
            assert ((order_quantity > 0) | order_quantity.isna()).all(), (
                "'order_quantity' values must be positive or nan"
            )
        else:
        ...
        if first_only or long_only or short_only:
            ...
            filtered_trades = filtered_orders.shift(
                1, fill_value=False if opposite_side is not None else True
            )
            ...
            order_mask = (
                trade_side_ffill.fillna(0).ne(0)
                & (~self["_signal_change"])
                & filtered_orders
            )
            ...
            if long_only or short_only:
                ...
                self["order_size"] = np.where(
                    filtered_orders & (self["order_price"].notna()),
                    0,
                    self["order_size"],
                )
                self["trade_size"] = np.where(
                    filtered_trades & (self["trade_price"].notna()),
                    0,
                    self["trade_size"],
                )
    ...
    def close_position(
        ...
        def _calculate_stop_price():
            ...
            market_order_trade_condition = (
                ...
                & (
                    ...
                    | (opened_order_price > self["high"])
                )
            ) | (
                short_order
                & (
                    (prev_close >= opened_order_price)
                    | (opened_order_price < self["low"])
                )
            )
            # Limit order: one-sided check suffices since gap-through is absorbed into market.
            ...
            for tp_or_sl, sign in [(stop_loss, -1), (take_profit, 1)]:
                ...
                gap_through_stop = (
                    negative_sign
                    & ~regular_immediate_stop
                    & (self["open"] < opened_stop_order_price)
                ) | (
                    positive_sign
                    & ~regular_immediate_stop
                    & (self["open"] > opened_stop_order_price)
                ...
                self["stop_price"] = np.where(
                    should_set_stop, opened_stop_order_price, self["stop_price"]
                )
                self["_immediate_stop"] = np.where(
                    should_set_stop,
                    stop_market_order_triggered_condition_immediate,
                    self["_immediate_stop"],
                )
                self["_gap_through_stop"] = np.where(
                    should_set_stop, gap_through_stop, self["_gap_through_stop"]
                )
            self["stop_price"] = self["stop_price"].shift(
                -1
            )  # shift back stop_price from trade row to order row
            self["_immediate_stop"] = self["_immediate_stop"].shift(
                -1, fill_value=False
            )
            self["_gap_through_stop"] = self["_gap_through_stop"].shift(
                -1, fill_value=False
            )
            ...
            if time_window:
                ...
                streak_start_bar_count = (
                    global_bar_count.shift(1, fill_value=0)
                    .where(self["_position_change"])
                    .ffill()
                    .fillna(0)
                )
        ...
        streak_start_cumsum = (
            global_cumsum.shift(1, fill_value=0)
            .where(self["_position_change"])
            .ffill()
            .fillna(0)
        )
        ...
        streak_start_cost_cumsum = (
            global_cost_cumsum.shift(1, fill_value=0)
            .where(self["_position_change"])
            .ffill()
            .fillna(0)
        )
        self["_agg_costs"] = global_cost_cumsum - streak_start_cost_cumsum
        self["avg_price"] = np.where(
            self["position"] != 0, self["_agg_costs"] / self["position"], np.nan
        )
        ...
        if has_close_condition:
            ...
            self["stop_price"] = np.where(
                self["_first_stop_order"], self["stop_price"], np.nan
            )
            ...
            if take_profit or stop_loss:
                ...
                price_diff_check = end_position_side * (
                    self["avg_price"] - self["stop_price"]
                )
                self["_stop_loss"] = (
                    self["_first_stop_order"] & not_time_window & (price_diff_check > 0)
                )
                self["_take_profit"] = (
                    self["_first_stop_order"] & not_time_window & (price_diff_check < 0)
                )
            ...
            self["trade_size"] = np.where(
                first_stop_trade, offset_size, self["trade_size"]
            )
            ...
            is_market_stop_trade = (
                self["_immediate_stop"].shift(1, fill_value=False) & ~is_gap_stop_trade
            )
            if time_window:
                is_market_stop_trade = is_market_stop_trade | self[
                    "_time_window"
            ...
            if fill_price == "open":
                ...
                stop_fill = np.where(
                    is_market_stop_trade, self["close"].shift(1), stop_fill
                )
            self["trade_price"] = np.where(
                first_stop_trade, stop_fill, self["trade_price"]
            )
            ...
            order_mask = (
                ...
                & (self["signal"] == np.sign(stop_side_ffill))
                & self["stop_price"].isna()
            )
            self["order_size"] = np.where(order_mask, np.nan, self["order_size"])
            self["order_price"] = np.where(order_mask, np.nan, self["order_price"])
            position_mask = first_stop_order_forwards_mask & (
                ~self["_first_stop_order"]
            )
