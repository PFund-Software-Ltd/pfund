...
class BacktestMixin:
    ...
    def backtest(self, df: IntoDataFrame) -> IntoDataFrame:
        if self.backtest_mode == BacktestMode.VECTORIZED:
            if hasattr(super(), "backtest"):
                ...
                self.store._df = self._vectorized_backtest(df)
    ...
    def _vectorized_backtest(self, X: IntoDataFrame) -> nw.DataFrame[Any]:
        ...
        trading_df = self.get_df(kind="trading", window_size=None, to_native=False)
        return nw.concat([trading_df, signals_df], how="horizontal")
    ...
    def _event_driven_backtest(self, df: IntoDataFrame) -> IntoDataFrame:
    ...
    # TODO: support all child/parent dataframe-form combinations; currently only
    # long-form child signals into a long-form component are implemented.
    def _merge_signals_dfs(self) -> nw.DataFrame[Any]:
        """Build this component's historical feature frame.

        The component's own data establishes its receiving bar spine. Each
        child component's signal dataframe is then aligned to that spine and
        merged as feature columns. Child signals can add values but cannot add
        rows to the receiving ``(date, product, resolution)`` universe.

        Strictly equal resolutions use exact alignment; other resolutions use
        close-time as-of alignment.
        """

        def _normalize_child_signals_form(
            child: Component,
        ...
        def _join_child_signals(
            ...
            child_value_cols = [
                col for col in child_signals.columns if col not in BAR_KEY_COLS
            ]
            conflicting_cols = [
        ...
        for child in self.components:
            child_signals = child.get_df(
                kind="signals", window_size=None, to_native=False
            )
            child_signals = _normalize_child_signals_form(child, child_signals)
            df = _join_child_signals(df, child, child_signals)

        return reorder_key_cols(df)
