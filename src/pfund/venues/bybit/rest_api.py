...
if TYPE_CHECKING:
    ...
    from pfund._apis.rest_api_base import ApiResponse, Result
...
from pfund.venues.bybit.venue import ProductCategory
from pfund._apis.rest_api_base import BaseRESTfulAPI, RequestMethod
from pfund.enums import CryptoAssetType, Environment, OptionType
...
class RESTfulAPI(BaseRESTfulAPI):
    ...
    PRIVATE_ENDPOINTS = {
        ...
        "switch_margin_mode": (
            RequestMethod.POST,
            f"/{VERSION}/position/switch-isolated",
        ),
        "switch_position_mode": (
            RequestMethod.POST,
            f"/{VERSION}/position/switch-mode",
        ),
    ...
    def _build_request(
        ...
        headers: dict | None = (
            self._authenticate(account, method, params=params) if account else None
        )
    ...
    def _authenticate(
        self, account: CryptoAccount, method: RequestMethod, params: dict | None = None
    ) -> dict:
    ...
    async def get_markets(
        self, category: ProductCategory | tProductCategory
    ) -> Result | ApiResponse:
        ...
        schema = {
            ...
            "data": {
                ...
                "asset_type": [
                    ...
                    lambda asset_type: (
                        CryptoAssetType[asset_type.upper()]
                        if asset_type in CryptoAssetType.__members__
                        else asset_type
                    ),
                ],
                ...
                "expiration": (
                    ...
                    lambda expiration: (
                        ...
                        if expiration == "0"
                            ...
                            int(expiration) / 1000, tz=datetime.UTC
                        )
                    ),
                ),
                "category": category,
        ...
        result: Result | ApiResponse = await self._request(
            endpoint_name, schema, params=params
        )
        return result
