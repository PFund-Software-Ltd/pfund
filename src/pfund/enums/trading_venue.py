...
class TradingVenue(StrEnum):
    ...
    def order_class(self) -> type[BaseOrder]:
        ...
        Order = getattr(
            importlib.import_module(f"pfund.entities.orders.order_{self.lower()}"),
            class_name,
        )
        return Order
    ...
    def account_class(self) -> type[BaseAccount]:
        if self == TradingVenue.IBKR:
            ...
            return getattr(
                importlib.import_module("pfund.entities.accounts.account_crypto"),
                class_name,
            )
        else:
            class_name = f"{self.capitalize()}Account"
        Account = getattr(
            importlib.import_module(f"pfund.entities.accounts.account_{self.lower()}"),
            class_name,
        )
        return Account
