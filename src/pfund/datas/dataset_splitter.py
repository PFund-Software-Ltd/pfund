...
class DatasetSplitter:
    ...
    def __post_init__(self):
        ...
        if isinstance(self.dataset_splits, (int, dict)):
            ...
            dataset_periods: list[CrossValidatorDatasetPeriods] = (
                self._split_by_cross_validator()
            )
            object.__setattr__(self, "dataset_periods", dataset_periods)
        else:
            raise ValueError(
                f"dataset_splits must be an int, a dict, or a TimeSeriesSplit, but got {type(self.dataset_splits)}"
            )
    ...
    def _split_by_ratio(self) -> DatasetPeriods:
        ...
        if isinstance(self.dataset_splits, int):
            ...
            assert len(digits) == 3, (
                '`dataset_splits` must be a number of length 3, e.g. "721" means 70% train, 20% val, 10% test'
            )
    ...
    def _split_by_cross_validator(self) -> list[CrossValidatorDatasetPeriods]:
        ...
        for fold_num, (train_indices, dev_indices) in enumerate(
            cross_validator.split(range(cv_days))
        ):
            dataset_periods_per_fold.append(
                {
                    ...
                    "train_set": (
                        cv_dates[train_indices[0]],
                        cv_dates[train_indices[-1]],
                    ),
