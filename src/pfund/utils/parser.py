    ...
    **Example:**
        ...
        try:
            ...
            for key, path in schema.items():
                ...
                # Case 2: Nested schema
                    ...
                    key_to_source = f"@{key}"
                    assert key_to_source in schema, (
                        f'"{key_to_source}" must be defined for nested schema'
                    )
    ...
    def convert(api_response: dict | list[dict], schema: dict) -> dict | list[dict]:
        if isinstance(api_response, dict):
            ...
            result: list[dict] = [
                SchemaParser._parse(item, schema) for item in api_response
            ]
            return result
        else:
            raise ParseApiResponseError(
                f'Unhandled API response type "{type(api_response)}":\n{api_response}'
            )
