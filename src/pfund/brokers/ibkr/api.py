...
if TYPE_CHECKING:
    ...
    from pfund.typing import AccountName, FullDataChannel, ProductName, tEnvironment
    ...
    def add_product(self, product: IBKRProduct) -> IBKRProduct:
        if product.name not in self._products:
            ...
            if existing_product != product:
                raise ValueError(
                    f"product {product.name} has already been used for {existing_product}"
                )
        return product
    ...
    def _create_public_channel(self, product: IBKRProduct, resolution: Resolution):
        ...
        if resolution.is_quote():
            ...
            if timeframe not in self.SUPPORTED_RESOLUTIONS:
                raise ValueError(
                    f"{self.exch} ({channel}.{product.symbol}) {resolution=} (timeframe={timeframe.name}) is not supported, supported timeframes: {[tf.name for tf in self.SUPPORTED_RESOLUTIONS]}"
                )
            elif period not in self.SUPPORTED_RESOLUTIONS[timeframe]:
                raise ValueError(
                    f"{self.exch} ({channel}.{product.symbol}) {resolution=} ({period=}) is not supported, supported periods: {self.SUPPORTED_RESOLUTIONS[timeframe]}"
                )
