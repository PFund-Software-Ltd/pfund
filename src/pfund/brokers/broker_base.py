...
from pfund.enums import Broker, CryptoExchange, Environment, TradingVenue
...
class BaseBroker(ABC):
    ...
    def __init__(self, env: Environment | str):
        ...
        self._products: defaultdict[CryptoExchange, dict[ProductName, BaseProduct]] = (
            defaultdict(dict)
        )
        self._accounts: defaultdict[TradingVenue, dict[AccountName, BaseAccount]] = (
            defaultdict(dict)
        )

        self._order_manager = OrderManager(self)
        self._portfolio_manager: PortfolioManager[BaseBalance, BasePosition] = (
            PortfolioManager()
        )
    ...
    @classmethod
    def create_product(
        cls, basis: str, exch: str = "", name: str = "", symbol: str = "", **specs
    ) -> BaseProduct:
        ...
        if cls.NAME == Broker.CRYPTO:
            assert exch in CryptoExchange.__members__, (
                f"{exch} is not a valid crypto exchange"
            )
        ...
        return Product(
            basis=basis, exchange=exch, name=name, symbol=symbol, specs=specs
        )
    ...
    def balances(
        self,
    ) -> dict[TradingVenue, dict[AccountName, dict[Currency, BaseBalance]]]:
        return self._portfolio_manager._balances

    @property
    def positions(
        self,
    ) -> dict[TradingVenue, dict[AccountName, dict[ProductName, BasePosition]]]:
    ...
    def add_public_channel(
        self, channel: PublicDataChannel, product: BaseProduct, resolution: Resolution
    ):
        pass
