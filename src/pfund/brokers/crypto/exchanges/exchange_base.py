...
if TYPE_CHECKING:
    ...
    from pfund.brokers.crypto.exchanges.rest_api_base import (
        BaseRESTfulAPI,
        Result,
    )
...
from pfund.enums import Broker, Environment, PrivateDataChannel, PublicDataChannel
...
class BaseExchange(ABC):
    ...
    def __init__(self, env: Environment | str):
        ...
        RESTfulAPI: type[BaseRESTfulAPI] = importlib.import_module(
            f"{exchange_path}.rest_api"
        ).RESTfulAPI
        self._rest_api: BaseRESTfulAPI = RESTfulAPI(self._env)
        WebSocketAPI: type[BaseWebSocketAPI] = importlib.import_module(
            f"{exchange_path}.ws_api"
        ).WebSocketAPI
    ...
    def get_file_path(cls, filename: str) -> Path | None:
        ...
        config = get_config()
        file_paths = {cls.MARKET_CONFIGS_FILENAME: config.cache_path / cls.NAME}
    ...
    def _check_if_refetch_market_configs(self):
        ...
        if force_refetching:
            ...
            self._logger.debug(
                f"{self.name} {filename} does not exist, fetching data..."
            )
            is_refetching = True
        elif is_outdated:
    ...
    ************************************************
    ...
    def cancel_batch_orders(
        self, account: CryptoAccount, scheme, params=None, **kwargs
    ):
        pass
