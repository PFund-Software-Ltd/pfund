...
if TYPE_CHECKING:
    from pfund.brokers.crypto.exchanges.rest_api_base import Result
...
class Exchange(BaseExchange):
    ...
    def get_supported_resolutions(
        ...
        WebSocketAPI: type[BaseWebSocketAPI] = getattr(
            importlib.import_module(
                f"pfund.brokers.crypto.exchanges.{name}.ws_api_{Category.lower()}"
            ),
            f"{Category}WebSocketAPI",
        )
    ...
    async def aget_markets(
        self, category: tProductCategory = ""
    ) -> dict[ProductCategory, Result]:
        categories = (
            [ProductCategory[category.upper()]]
            if category
            else [category for category in ProductCategory]
        )
    ...
    def cancel_order(
        self, account: CryptoAccount, product: BybitProduct, order: CryptoOrder
    ):
        schema = {
            "@data": "result",
            "ts": "time",
    ...
    def place_batch_orders(
        self, account: CryptoAccount, product: BybitProduct, orders: list[CryptoOrder]
    ):
