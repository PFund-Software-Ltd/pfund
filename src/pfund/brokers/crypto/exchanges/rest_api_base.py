...
from pfund_kit.utils.yaml import dump, load

from pfund.enums import CryptoExchange, Environment
...
class BaseRESTfulAPI(ABC):
    ...
    PUBLIC_ENDPOINTS: ClassVar[
        dict[EndpointName, tuple[RequestMethod, EndpointPath]]
    ] = {}
    PRIVATE_ENDPOINTS: ClassVar[
        dict[EndpointName, tuple[RequestMethod, EndpointPath]]
    ] = {}
    ...
    def __init__(self, env: Environment | tEnvironment):
        ...
        Exchange: type[BaseExchange] = (
            importlib.import_module(
                f"pfund.brokers.crypto.exchanges.{self.exch.lower()}.exchange"
            )
        ).Exchange
    ...
    def list_endpoints(self, endpoint_type: Literal["public", "private"]) -> None:
        ...
        endpoints = (
            ...
            if endpoint_type.lower() == "public"
            else self.PRIVATE_ENDPOINTS
        )
        pprint(endpoints)
    ...
    def get_endpoint(self, endpoint_name: EndpointName) -> tuple[RequestMethod, str]:
        ...
        if self._env == Environment.SANDBOX and is_public_endpoint:
            ...
            self._logger.warning(
                f'{self._env} environment is using LIVE data for public endpoint "{endpoint_name}"'
            )
        else:
    ...
    def _append_sample_return(
        self, endpoint_name: EndpointName, api_response: ApiResponse
    ):
    ...
    async def _request(
        ...
        request: Request = self._build_request(
            method=method, endpoint=endpoint, account=account, params=params
        )
        result: Result = {
            "is_success": False,
            "error": None,
        ...
        try:
            ...
            parsed_data: dict | list[dict] = (
                SchemaParser.convert(api_response, schema)
                if is_success
                else api_response
            )
            ...
            self._logger.exception(
                f'Unhandled REST API exception when calling "{endpoint_name}":'
            )
        return result
