...
class BaseWebSocketAPI(ABC):
    ...
    def __init__(self, env: Environment | tEnvironment):
        ...
        self._callback: (
            Callable[[WebSocketName, Message], Awaitable[None] | None] | None
        ) = None
    ...
    def _get_url(
        self, channel_type: DataChannelType | Literal["public", "private"]
    ) -> str:
    ...
    async def _connect_ws(self, url: str, account: CryptoAccount | None = None):
        ...
        try:
            ...
            self._logger.warning(
                f"{ws_name} failed to connect ({err=}), will try again later"
            )
