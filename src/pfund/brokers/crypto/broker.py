...
class CryptoBroker(BaseBroker):
    ...
    def add_private_channel(
        self, exch: CryptoExchange, channel: PrivateDataChannel | FullDataChannel
    ):
        exchange = self.get_exchange(exch)
        ...
        **kwargs,
        ...
        if not is_api_call:
            return self._order_manager.get_orders(
                exch, acc, pdt=pdt, oid=oid, eoid=eoid
            )
        else:
            exchange = self.get_exchange(exch)
            account = self.get_account(exch, acc)
    ...
    def get_trades(
        self, exch: CryptoExchange, acc: str, pdt: str = "", is_api_call=False, **kwargs
    ) -> dict | None:
        exch, acc, pdt = to_uppercase(exch, acc, pdt)
        if not is_api_call:
    ...
    def get_balances(
        ...
        **kwargs: Any,
        ...
        ccy = currency.upper()
        if not is_api_call:
            return self._portfolio_manager.get_balances(
                venue=venue, acc=account_name, ccy=currency
            )
        else:
            exchange = self.get_exchange(exch)
            account = self.get_account(exch, acc)
    ...
    def place_orders(
        self, account: CryptoAccount, product: CryptoProduct, orders: list[BaseOrder]
    ):
