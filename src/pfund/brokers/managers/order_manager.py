...
class OrderManager:
    ...
    def get_orders(self, venue, acc, pdt="", oid="", eoid="", is_opened_only=False):
        if oid or eoid:
            ...
            return (
                orders if not pdt else [order for order in orders if order.pdt == pdt]
            )
        ...
        for pdt, updates in data.items():
            ...
            for o in opened_orders:
                if o.eoid and o.eoid not in official_eoids:
                    ...
                    self._logger.debug(
                        f"{venue} {acc=} {o.oid=} {o.eoid=} add recon_num to {self._recon_nums[o.oid]}"
                    )
                    if self._recon_nums[o.oid] >= self._MAX_RECON_NUM:
                        self._logger.warning(
                            f"cancel order {o.oid} due to reconciliation"
                        )
            ...
            for acc in self.submitted_orders[tv]:
                ...
                for o in submitted_orders:
                    ...
                    if submitted_ts - now > self._NO_ORDER_UPDATE_TOLERANCE_IN_SECONDS:
                        ...
                        self._logger.debug(
                            f"{tv} {acc=} {o.oid} add missed_num to {self._missed_nums[o.oid]}"
                        )
                        if self._missed_nums[o.oid] >= self._MAX_MISSED_NUM:
                            self._logger.warning(
                                f"cancel order {o.oid} due to missing update"
                            )
                            self._broker.cancel_orders(o.account, o.product, [o])
    ...
    def update_orders(self, venue, acc, orders: dict):
        ...
        for pdt, updates in data.items():
            for update in updates:
                if (
                    not (
                        order := self.get_order(
                            venue, acc, oid=update["oid"], eoid=update["eoid"]
                        )
                    )
                    and source == OrderUpdateSource.GOO
                ):
                    ...
                    if source == OrderUpdateSource.GTH:
                        ...
                        self._logger.error(
                            f"Cannot find order {venue=} {acc=} {pdt} {update=}"
                        )
                    return
                ...
                if "status" in update:
                    main_status, fill_status, cancel_status, amend_status = update[
                        "status"
                    ]
                else:
                ...
                if fill_status in ["P", "F"] or source in [
                    OrderUpdateSource.GTH,
                    OrderUpdateSource.WST,
                ]:
                    self._on_trade(
                        order,
                        fill_status,
    ...
    def on_submitted(self, order, ts=None, reason=""):
        if is_updated := order.on_status_update(
            status=MainOrderStatus.SUBMITTED, ts=ts, reason=reason
        ):
    ...
    def _on_opened(self, order, ts=None, reason=""):
        if order.oid in self.closed_orders[order.venue][order.acc]:
            self._logger.warning(
                f"closed order {order} is trying to be re-opened, probably due to a delayed update"
            )
            return
        if is_updated := order.on_status_update(
            status=MainOrderStatus.OPENED, ts=ts, reason=reason
        ):
    ...
    def _on_closed(self, order, ts=None, reason: OrderClosedReason | str = ""):
        ...
        if (
            order.oid not in self.opened_orders[order.venue][order.acc]
            and reason != MainOrderStatus.REJECTED
        ):
            self._on_opened(order, ts=ts, reason="open to be closed")
        if is_updated := order.on_status_update(
            status=MainOrderStatus.CLOSED, ts=ts, reason=reason
        ):
    ...
    def _on_rejected(self, order, ts=None, reason=""):
        if is_updated := order.on_status_update(
            status=MainOrderStatus.REJECTED, ts=ts, reason=reason
        ):
    ...
    def _on_trade(
        ...
        if is_updated := order.on_trade_update(
            avg_px, filled_qty, last_traded_px, last_traded_qty
        ):
    ...
    def _on_filled(self, order, ts=None, reason=""):
        if is_updated := order.on_status_update(
            status=FillOrderStatus.FILLED, ts=ts, reason=reason
        ):
    ...
    def _on_cancelled(self, order, ts=None, reason=""):
        if is_updated := order.on_status_update(
            status=CancelOrderStatus.CANCELLED, ts=ts, reason=reason
        ):
