...
def OrderFactory(
    venue: TradingVenue | str, order_type: OrderType | str
) -> type[BaseOrder]:
    import importlib

    VenueClass = TradingVenue[venue.upper()].venue_class
    Order = VenueClass.Order
    order_type = OrderType[order_type.upper()]
    OrderTypeMixin = getattr(
        importlib.import_module(
            f"pfund.entities.orders.mixins.{order_type.lower()}_order"
        ),
        f"{order_type.capitalize()}OrderMixin",
    )
    class_name = (
