...
class Portfolio(AllAssetsMixin, BasePortfolio):
    ...
    def initialize(self, positions: list[BasePosition], balances: list[BaseBalance]):
        for bkr in {pb.bkr for pb in positions + balances}:
            positions_per_bkr = [
                position for position in positions if position.bkr == bkr
            ]
            balances_per_bkr = [balance for balance in balances if balance.bkr == bkr]
            portfolio: BasePortfolio = self._add_sub_portfolio(
                bkr, positions_per_bkr, balances_per_bkr
            )
        ...
        for attr in (
            ...
            setattr(
                ...
                self.combine_dicts(
                    *(
                        getattr(pfo, attr)
                        for pfo in self._sub_portfolios.values()
                        if hasattr(pfo, attr)
                    )
                ),
            )
    ...
    def _add_sub_portfolio(
        self, bkr: str, positions: list[BasePosition], balances: list[BaseBalance]
    ) -> BasePortfolio:
        if bkr not in self._sub_portfolios:
            if bkr == "CRYPTO":
                portfolio = CeFiPortfolio.from_positions_and_balances(
