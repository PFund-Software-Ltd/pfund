...
class BasePortfolio:
    ...
    def get_positions(
        self, ptype: str, account: BaseAccount | None = None, pdt: str = ""
    ) -> (
        defaultdict[str, dict[str, BasePosition]]
        | dict[str, BasePosition]
        | BasePosition
        | None
    ):
        ...
        if not account and not pdt:
            ...
            return {
                account: position
                for account, pdt_to_position in assets.items()
                for position in pdt_to_position.values()
                if position.pdt == pdt
            }

    def get_balances(
        self, account: BaseAccount | None = None, ccy: str = ""
    ) -> (
        defaultdict[str, dict[str, BaseBalance]]
        | dict[str, BaseBalance]
        | BaseBalance
        | None
    ):
        ...
        if not account and not ccy:
            ...
            return {
                account: balance
                for account, ccy_to_balance in balances.items()
                for balance in ccy_to_balance.values()
                if balance.ccy == ccy
            }
    ...
    def has_balance(self, balance: BaseBalance) -> bool:
        return (
            balance.account in self._currencies
            and balance.ccy in self._currencies[balance.account]
        )
