...
class IBKRAccount(BaseAccount):
    ...
    def __init__(
        ...
        self._client_id = client_id or os.getenv(
            f"{self.venue}_CLIENT_ID", self._next_default_client_id()
        )
        if self._client_id:
            self._client_id = int(self._client_id)
        if self._env in [Environment.SANDBOX, Environment.PAPER, Environment.LIVE]:
            assert self._host, (
                f"{self.venue} host must be provided, please set `{self.venue}_HOST` in .env.{self._env.lower()} file, or in add_account(..., host=...)."
            )
            ...
            assert self._client_id, (
                f"{self.venue} client id must be provided, please set `{self.venue}_CLIENT_ID` in .env.{self._env.lower()} file, or in add_account(..., client_id=...)."
            )
