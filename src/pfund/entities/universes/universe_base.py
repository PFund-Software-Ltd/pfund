...
class BaseUniverse:
    ...
    def get(
        self, ptype: str, exch: str = "", pdt: str = ""
    ) -> (
        defaultdict[str, dict[str, BaseProduct]]
        | dict[str, BaseProduct]
        | BaseProduct
        | None
    ):
        ...
        if not exch and not pdt:
            ...
            return {
                exch: product
                for exch, pdt_to_product in assets.items()
                for product in pdt_to_product.values()
                if str(product) == pdt
            }
