...
def vectorized(func):
    def wrapper(*args, **kwargs):
        ...
        if BacktestEngine.mode == BacktestMode.vectorized:
...
def event_driven(func):
    def wrapper(*args, **kwargs):
        # FIXME
        self: BacktestMixin | BaseStrategy | BaseModel = args[0]
        if BacktestEngine.mode == BacktestMode.event_driven:
            return func(*args, **kwargs)
        else:
...
class BacktestMixin:
    ...
    def dataset_periods(self: BacktestMixin | BaseStrategy | BaseModel) -> DatasetPeriods | list[CrossValidatorDatasetPeriods]:
        return BacktestEngine.dataset_periods
    ...
    def on_stop(self: BaseStrategy | BaseModel):
        super().on_stop()
        if BacktestEngine.mode == BacktestMode.event_driven and BacktestEngine._assert_signals and self._has_signal_df():
    ...
    def _prepare_df(self: BacktestMixin | BaseStrategy | BaseModel):
        if self._is_dummy_strategy and isinstance(self, BaseStrategy):
            return
        ts_col_type = 'timestamp' if BacktestEngine.mode == BacktestMode.event_driven else 'datetime'
    ...
    def clear_dfs(self: BacktestMixin | BaseStrategy | BaseModel):
        assert BacktestEngine.mode == BacktestMode.event_driven
        if not self._is_signal_df_required:
