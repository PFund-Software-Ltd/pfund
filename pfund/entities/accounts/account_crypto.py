...
class CryptoAccount(BaseAccount):
    def __init__(self, env: Environment | str, exchange: CryptoExchange | str, name: str='', key: str='', secret: str=''):
        ...
        if self._env in [Environment.PAPER, Environment.LIVE]:
            assert self._key, f'{self.venue} API key must be provided, please set `{self.venue}_API_KEY` in .env.{self._env.lower()} file, or in add_account(..., key=...).'
            assert self._secret, f'{self.venue} API secret must be provided, please set `{self.venue}_API_SECRET` in .env.{self._env.lower()} file, or in add_account(..., secret=...).'
