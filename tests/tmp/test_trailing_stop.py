...
def _make_df(
    n: int, *, open=None, high=None, low=None, close=None, volume=None
) -> pd.DataFrame:
    return pd.DataFrame(
        {
...
class TestBestPriceReset:
    def test_best_price_resets_on_new_position(self):
        ...
        assert result["position"].iloc[4] == 1.0, (
            f"Bar 4: second long should open, got {result['position'].iloc[4]}"
        )
        assert result["position"].iloc[5] == 1.0, (
            f"Bar 5: should stay open (48 > 47.5), got {result['position'].iloc[5]}"
        )
