...
def _get_data_file_path(
    data_source, pdt_or_symbol, resolution, rollback_period, start_date, end_date
):
...
def yf_data(mocker, request):
    ...
    mock_feed.get_historical_data.side_effect = lambda *args, **kwargs: (
        get_historical_data(*args, **kwargs, data_source=data_source)
    )
