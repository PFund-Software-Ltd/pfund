...
class TestCore:
    def test_vectorized_backtesting_flow(self, mocker):
        ...
        strategy = engine.add_strategy(
            FakeVectorizedStrategy(), name="fake_vectorized_strategy", is_parallel=False
        )
        ...
        # add pytorch model:
        strategy.add_model(
            DummyPytorch(ml_model=PytorchLinearRegression()), name="dummy_pytorch"
        )

        # add sklearn model:
        strategy.add_model(
            DummySklearn(ml_model=SklearnLinearRegression()), name="dummy_sklearn"
        )

        # add ta indicators
        ## type 1: ta class, e.g. ta.volatility.BollingerBands
        indicator = lambda df: ta.volatility.BollingerBands(
            close=df["close"], window=3, window_dev=2
        )
        funcs = ["bollinger_mavg", "bollinger_hband", "bollinger_lband"]
        strategy.add_indicator(
            ind.TaIndicator(indicator, funcs=funcs), name="BollingerBands"
        )
        ## type 2: ta function, e.g. ta.volatility.bollinger_mavg
        indicator2 = lambda df: ta.volatility.bollinger_mavg(
            close=df["close"], window=3
        )
        ...
        strategy_datas = [
            data
            for data_per_resolution in strategy.datas.values()
